...
export async function* parseChatGpt(
  ...
  if (!isPlausibleConversationsArray(data)) {
    throw new MalformedExportError(
      `${CONVERSATIONS_FILE} does not match the expected ChatGPT export structure ` +
        `(expected a JSON array of conversation objects with a "mapping" field). ` +
        `This export may be newer or older than the installed ChatGPT adapter supports.`,
    );
  }
