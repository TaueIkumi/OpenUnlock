...
import { linearAdapter } from "../../src/adapters/linear/index.js";
...
const fixtureDir = path.join(testDir, "../../src/adapters/linear/fixtures/basic");
const snapshotsDir = path.join(testDir, "../snapshots/linear/basic");

describe("linear basic fixture end-to-end", () => {
  it("produces deterministic, golden-matching JSON output", async () => {
    const outDir = await mkdtemp(path.join(tmpdir(), "openunlock-linear-json-"));
    try {
      const input = await openInputSource(fixtureDir);
      const result = await runPipeline(input, linearAdapter, jsonExporter, {
        outputDir: outDir,
        overwrite: true,
        dryRun: false,
      });
  ...
  it("produces deterministic, golden-matching Markdown output", async () => {
    const outDir = await mkdtemp(path.join(tmpdir(), "openunlock-linear-md-"));
    try {
      const input = await openInputSource(fixtureDir);
      await runPipeline(input, linearAdapter, markdownExporter, {
        outputDir: outDir,
        overwrite: true,
        dryRun: false,
      });
  ...
  it("passes doctor validation for the filesystem exporter", async () => {
    const outDir = await mkdtemp(path.join(tmpdir(), "openunlock-linear-fs-"));
    try {
      const input = await openInputSource(fixtureDir);
      await runPipeline(input, linearAdapter, filesystemExporter, {
        outputDir: outDir,
        overwrite: true,
        dryRun: false,
      });
  ...
  it("converts a loose issues.csv file directly (no wrapping directory)", async () => {
    const outDir = await mkdtemp(path.join(tmpdir(), "openunlock-linear-loose-"));
    try {
      const input = await openInputSource(path.join(fixtureDir, "issues.csv"));
      const result = await runPipeline(input, linearAdapter, jsonExporter, {
        outputDir: outDir,
        overwrite: true,
        dryRun: false,
      });
      expect(result.exportResult.entityCounts.document).toBe(2);
    } finally {
      await rm(outDir, { recursive: true, force: true });
    }
  });
