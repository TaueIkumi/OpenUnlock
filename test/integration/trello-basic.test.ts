...
import { trelloAdapter } from "../../src/adapters/trello/index.js";
...
const fixtureDir = path.join(testDir, "../../src/adapters/trello/fixtures/basic");
const snapshotsDir = path.join(testDir, "../snapshots/trello/basic");

describe("trello basic fixture end-to-end", () => {
  it("produces deterministic, golden-matching JSON output", async () => {
    const outDir = await mkdtemp(path.join(tmpdir(), "openunlock-trello-json-"));
    try {
      const input = await openInputSource(fixtureDir);
      const result = await runPipeline(input, trelloAdapter, jsonExporter, {
        outputDir: outDir,
        overwrite: true,
        dryRun: false,
      });
  ...
  it("produces deterministic, golden-matching Markdown output", async () => {
    const outDir = await mkdtemp(path.join(tmpdir(), "openunlock-trello-md-"));
    try {
      const input = await openInputSource(fixtureDir);
      await runPipeline(input, trelloAdapter, markdownExporter, {
        outputDir: outDir,
        overwrite: true,
        dryRun: false,
      });
  ...
  it("passes doctor validation with checksums for the filesystem exporter", async () => {
    const outDir = await mkdtemp(path.join(tmpdir(), "openunlock-trello-fs-"));
    try {
      const input = await openInputSource(fixtureDir);
      await runPipeline(input, trelloAdapter, filesystemExporter, {
        outputDir: outDir,
        overwrite: true,
        dryRun: false,
      });
  ...
  it("converts a loose board.json file directly (no wrapping directory)", async () => {
    const outDir = await mkdtemp(path.join(tmpdir(), "openunlock-trello-loose-"));
    try {
      const input = await openInputSource(path.join(fixtureDir, "board.json"));
      const result = await runPipeline(input, trelloAdapter, jsonExporter, {
        outputDir: outDir,
        overwrite: true,
        dryRun: false,
