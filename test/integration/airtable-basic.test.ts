...
import { airtableAdapter } from "../../src/adapters/airtable/index.js";
...
const fixtureDir = path.join(testDir, "../../src/adapters/airtable/fixtures/basic");
const snapshotsDir = path.join(testDir, "../snapshots/airtable/basic");

describe("airtable basic fixture end-to-end", () => {
  it("produces deterministic, golden-matching JSON output", async () => {
    const outDir = await mkdtemp(path.join(tmpdir(), "openunlock-airtable-json-"));
    try {
      const input = await openInputSource(fixtureDir);
      const result = await runPipeline(input, airtableAdapter, jsonExporter, {
        outputDir: outDir,
        overwrite: true,
        dryRun: false,
      });
  ...
  it("produces deterministic, golden-matching Markdown output", async () => {
    const outDir = await mkdtemp(path.join(tmpdir(), "openunlock-airtable-md-"));
    try {
      const input = await openInputSource(fixtureDir);
      await runPipeline(input, airtableAdapter, markdownExporter, {
        outputDir: outDir,
        overwrite: true,
        dryRun: false,
      });
  ...
  it("passes doctor validation for the filesystem exporter", async () => {
    const outDir = await mkdtemp(path.join(tmpdir(), "openunlock-airtable-fs-"));
    try {
      const input = await openInputSource(fixtureDir);
      await runPipeline(input, airtableAdapter, filesystemExporter, {
        outputDir: outDir,
        overwrite: true,
        dryRun: false,
      });
