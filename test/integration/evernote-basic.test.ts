...
const snapshotsDir = path.join(testDir, "../snapshots/evernote/basic");

describe("evernote basic fixture end-to-end", () => {
  it("produces deterministic, golden-matching JSON output", async () => {
    const outDir = await mkdtemp(path.join(tmpdir(), "openunlock-evernote-json-"));
    try {
      const input = await openInputSource(fixtureDir);
      const result = await runPipeline(input, evernoteAdapter, jsonExporter, {
        outputDir: outDir,
        overwrite: true,
        dryRun: false,
      });
  ...
  it("produces deterministic, golden-matching Markdown output, including a real copied attachment", async () => {
    const outDir = await mkdtemp(path.join(tmpdir(), "openunlock-evernote-md-"));
    try {
      const input = await openInputSource(fixtureDir);
      await runPipeline(input, evernoteAdapter, markdownExporter, {
        outputDir: outDir,
        overwrite: true,
        dryRun: false,
      });
      ...
      const expectedFiles = (await readdir(path.join(snapshotsDir, "markdown"))).filter((f) =>
        f.endsWith(".md"),
      ).sort();
      ...
      // The attachment's actual bytes must be copied, not just referenced.
  ...
  it("passes doctor validation with checksums for the filesystem exporter", async () => {
    const outDir = await mkdtemp(path.join(tmpdir(), "openunlock-evernote-fs-"));
    try {
      const input = await openInputSource(fixtureDir);
  ...
  it("detects tampering with a copied attachment's bytes", async () => {
    const outDir = await mkdtemp(path.join(tmpdir(), "openunlock-evernote-tamper-"));
    try {
      const input = await openInputSource(fixtureDir);
      await runPipeline(input, evernoteAdapter, filesystemExporter, {
        outputDir: outDir,
      ...
      const attachmentFiles = (await readdir(path.join(outDir, "attachments"))).filter(
        (f) => !f.endsWith(".json"),
      );
      expect(attachmentFiles).toHaveLength(1);
      const { writeFile } = await import("node:fs/promises");
      await writeFile(path.join(outDir, "attachments", attachmentFiles[0]!), "tampered bytes");

      const findings = await runDoctor(outDir);
      expect(
        findings.some((f) => f.severity === "error" && f.message.includes("Checksum mismatch")),
      ).toBe(true);
    } finally {
      await rm(outDir, { recursive: true, force: true });
    }
