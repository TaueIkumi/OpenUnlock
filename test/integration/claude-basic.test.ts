...
const fixtureDir = path.join(testDir, "../../src/adapters/claude/fixtures/basic");
const snapshotsDir = path.join(testDir, "../snapshots/claude/basic");

describe("claude basic fixture end-to-end", () => {
  it("produces deterministic, golden-matching JSON output", async () => {
    const outDir = await mkdtemp(path.join(tmpdir(), "openunlock-claude-json-"));
    try {
      const input = await openInputSource(fixtureDir);
      const result = await runPipeline(input, claudeAdapter, jsonExporter, {
        outputDir: outDir,
        overwrite: true,
        dryRun: false,
      });
  ...
  it("produces deterministic, golden-matching Markdown output", async () => {
    const outDir = await mkdtemp(path.join(tmpdir(), "openunlock-claude-md-"));
    try {
      const input = await openInputSource(fixtureDir);
      await runPipeline(input, claudeAdapter, markdownExporter, {
        outputDir: outDir,
        overwrite: true,
        dryRun: false,
      });
  ...
  it("passes doctor validation for the filesystem exporter", async () => {
    const outDir = await mkdtemp(path.join(tmpdir(), "openunlock-claude-fs-"));
    try {
      const input = await openInputSource(fixtureDir);
      await runPipeline(input, claudeAdapter, filesystemExporter, {
        outputDir: outDir,
        overwrite: true,
        dryRun: false,
      });
