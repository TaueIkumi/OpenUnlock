...
import { googleTasksAdapter } from "../../src/adapters/google-tasks/index.js";
...
const fixtureDir = path.join(testDir, "../../src/adapters/google-tasks/fixtures/basic");
const snapshotsDir = path.join(testDir, "../snapshots/google-tasks/basic");

describe("google-tasks basic fixture end-to-end", () => {
  it("produces deterministic, golden-matching JSON output", async () => {
    const outDir = await mkdtemp(path.join(tmpdir(), "openunlock-gtasks-json-"));
    try {
      const input = await openInputSource(fixtureDir);
      const result = await runPipeline(input, googleTasksAdapter, jsonExporter, {
        outputDir: outDir,
        overwrite: true,
        dryRun: false,
      });
  ...
  it("produces deterministic, golden-matching Markdown output", async () => {
    const outDir = await mkdtemp(path.join(tmpdir(), "openunlock-gtasks-md-"));
    try {
      const input = await openInputSource(fixtureDir);
      await runPipeline(input, googleTasksAdapter, markdownExporter, {
        outputDir: outDir,
        overwrite: true,
        dryRun: false,
      });
  ...
  it("passes doctor validation for the filesystem exporter", async () => {
    const outDir = await mkdtemp(path.join(tmpdir(), "openunlock-gtasks-fs-"));
    try {
      const input = await openInputSource(fixtureDir);
      await runPipeline(input, googleTasksAdapter, filesystemExporter, {
        outputDir: outDir,
        overwrite: true,
        dryRun: false,
      });
